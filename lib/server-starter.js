...
class Server extends EventEmitter {
  ...
  launch (cmd, args, options = {}) {
    ...
    return new Promise((resolve) => {
      const timeOut = 15000 // max time waiting for connection
      const retryTime = 60 // retry every this amount of mS
      const now = new Date()
      const timeToStop = new Date(now.getTime() + timeOut)
        ...
        connection.on('error', err => {
          if (err.code === 'ECONNREFUSED') {
            if (new Date() < timeToStop) {
              ...
              // console.log(`# waiting: ${((new Date()).getTime() - now.getTime()) / 1000} segs`)
            }
          }
        })
      })()
